...
.. confval:: breathe_use_cpp_namespace

   If set to ``True``, the ``cpp:namespace``, ``cpp:namespace-push`` and
   ``cpp:namespace-pop`` directives are observed.
   ...
   Similarly to the directives of the ``cpp`` domain, the current namespace is
   not included in the definition of the symbol in the output generated by
   Sphinx, but is still taken into account for cross-referencing.

   Additionally, this changes the default behavior of how ``doxygenclass``,
   ``doxygenstruct``, ``doxygennamespace`` and ``doxygenunion`` are presented.
   By default, the namespace is always omitted for these directives. When
   ``breathe_use_cpp_namespace`` is ``True``, they are treated like the other
   directives and include the namespace (as given by the ``doxygen-`` directive,
