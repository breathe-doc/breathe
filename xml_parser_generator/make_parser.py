...
BUILTIN_ATTR_SCHEMA_TYPES = [('string','str'),('DoxBool','bool'),('integer','int'),('empty','None')]
...
class ListElement(ElementType):
    ...
    def py_union_list(self) -> list[str]:
        ...
        types = ['TaggedValue[Literal[{}], {}]'.format(
            comma_join(sorted(f"'{n}'" for n in names),26),
            t) for t,names in by_type.items()]
        str_included = False
        for b in self.bases:
            if isinstance(b,ListElement):
                types.extend(b.py_union_ref())
                if b.allow_text: str_included = True
    ...
    for typename,t in schema.types.items():
        ...
        if isinstance(t,ElementType):
            ...
            for i,b in enumerate(t.bases):
                ...
                if isinstance(b_type,ListElement):
                    ...
                    if b_type.content_type != t.content_type:
                        raise ValueError(f'list elements of one type cannot use list elements of another type as bases')
                t.bases[i] = b_type
            for name,child in t.children.items():
                child.name = name
                if not child.py_name: child.py_name = name
            ...
            for name,attr in t.attributes.items():
                attr.name = name
                if not attr.py_name: attr.py_name = name
...
def collect_field_names(all_fields: set[str],cur_fields: set[str],refs: Iterable[Attribute|TypeRef],type_name: str) -> None:
    for ref in refs:
...
def make_env(schema: Schema) -> jinja2.Environment:
    ...
    def field_count(t) -> int:
        if not isinstance(t,ElementType): return 0
    ...
    def has_children_or_content(t):
        if not isinstance(t,ElementType):
            return False
        return t.children or (isinstance(t,ListElement) and t.content) or any(has_children_or_content(b) for b in t.bases)
    
    def has_children_or_tuple_content(t):
        if not isinstance(t,ElementType):
            return False
        return (t.children
                or (isinstance(t,ListElement) and t.content_type == ContentType.tuple and len(t.content) > 1)
    ...
    def list_type_or_base(t):
        if not isinstance(t,ElementType): return False
        return isinstance(t,ListElement) or any(list_type_or_base(b) for b in t.bases)

    def allow_text(t):
        if not isinstance(t,ListElement): return False
...
def get_json_value(conv: Callable[[Any,str],T],context: str,d: dict[str,Any],key:str,default: T|_NoDefault=_NO_DEFAULT) -> T:
...
def get_json_mapping(item_conv: Callable[[Any,str],T],context: str,d: dict,key: str,default: dict[str,T]|_NoDefault=_NO_DEFAULT) -> dict[str,T]:
    def check(x,context):
...
def get_json_list(item_conv: Callable[[Any,str],T],context: str,d: dict,key: str,default: list[T]|_NoDefault=_NO_DEFAULT) -> list[T]:
...
def check_other_attr_action(x,context:str) -> OtherAttrAction:
    if x == "ignore": return OtherAttrAction.ignore
    if x == "error": return OtherAttrAction.error
...
def check_type(x,context:str) -> SchemaType:
    ...
    raise ValueError(f'"{context}.kind" must be "tag_only_element", "list_element", "mixed_element" or "enumeration"')
...
def generate_from_json(json_path,c_template_file,pyi_template_file,c_output_file,pyi_output_file) -> None:
