...
class MemberDefTypeSubRenderer(Renderer):
    ...
    def objname(self):
        """Return the name of the rendered object."""
    ...
    def render(self):
        nodes = self.run_domain_directive(self.objtype(), [self.objname()])
        node = nodes[1]
...
class EnumMemberDefTypeSubRenderer(MemberDefTypeSubRenderer):
    ...
    def objname(self):
        ...
        name = self.data_object.name
        return name.replace("@", "__anonymous") if name.startswith("@") else name
